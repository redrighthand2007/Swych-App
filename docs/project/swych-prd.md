...
|---|---|---|
| Deal mechanism | (a) First-come-first-lock with an atomic Postgres transaction | **(b)** Multiple buyers submit offers, seller picks one — no race condition since the seller is the sole decision-maker |
...
- Hostel/block,phone numebr is self-declared, used for filtering and light proximity trust — not independently verified in v1.
